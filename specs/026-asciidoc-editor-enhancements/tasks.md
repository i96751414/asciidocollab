...
- [~] T052 [US8] Added `mainFileNodeId` to the `Project` entity (`packages/domain/src/entities/project.ts`, with `setMainFile()`) and to `ProjectDto` in `packages/shared`. **Prisma `schema.prisma` column + `onDelete: SetNull` and the dev `db push` (T052a) are NOT yet applied — deferred (needs a running dev Postgres; can't run in this environment).**
