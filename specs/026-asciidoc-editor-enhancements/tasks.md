...
- [X] T019 [US7] **Block-level grammar additions done** (`Conditional`, `BlockAttributeLine`, `CsvTableBlock`, `DsvTableBlock`). **Inline-construct rework now landed** via TDD (`asciidoc-grammar-us7-inline.test.ts`, 22 cases): `inlineWord` narrowed to exclude `+ [ ( &` (and `<`); dedicated single-shape tokens added for inline **passthrough** `+x+`, **inline/biblio anchors** `[[id]]`/`[[[ref]]]`, **replacements** `(C)(R)(TM)`, **entities** `&amp;`/`&#NNN;`, **code callouts** `<1>`; plus `punctFallback`/`ltFallback` so ordinary prose (arithmetic, `f(x)`, `Q&A`, `a < b`, `[note]`) still tokenizes with zero parser errors, and the same fallbacks added to emphasis `innerContent` (no `*a+b*` regression). Block-level **thematic break** `'''` and **page break** `<<<` added to the external tokenizer (production + test mirror). Parser regenerated; **584 codemirror + 1804 web unit tests green (no regression).** **Inline UI/math macros (FR-052) now also landed:** `kbd:[…]`/`btn:[…]` → `UiMacro`, `stem:[…]`/`latexmath:[…]`/`asciimath:[…]` → `InlineStem`, as `@tokens` with `@precedence { stemMark, uiMacroMark, inlineWord }` (precedence only fires when the full `name:[…]` shape matches; plain words fall to `inlineWord`). These are divergence-free because an immediate `[` is excluded from the external inline-macro path in both tokenizers. Mapped to `t.macroName`/`special(macroName)`; added `t.macroName` to the theme (also fixes BlockMacro, which was latently unstyled). **Bare-URL auto-links (FR-026) also landed:** `http(s)://`/`ftp://`/`irc://` → `Link` (→`t.link`) as a `@tokens` rule with `@precedence { …, bareUrl, inlineWord }`; the `://` scheme discriminator makes the precedence safe (a plain colon-bearing word like `note:x`/`3:45` never matches — covered by negative tests). **Smart typographic quotes (FR-054):** `"\`…\`"`/`'\`…\`'` → `SmartQuote` (→`t.quote`); `"`/`'` excluded from `inlineWord` + `quoteFallback` (also into `innerContent`) so plain `"hello"`/`don't` stay prose. **link:/mailto:/menu: + ALL mid-line macros + footnotes (FR-026/052):** added `inlineMacroMark`/`footnoteMark` `@token` ALTERNATIVES to `InlineMacro`/`Footnote` (`name:target[args]` single-colon discriminator) — this ALSO fixes a real pre-existing gap where the `isLineStart`-gated external tokenizer never tokenized mid-line inline-macros/footnotes in the live editor (external still wins at line start; the `@token` wins mid-line — same node, no regression). **Hard line break (FR-054):** trailing ` +` before EOL → `HardBreak` (→`t.escape`), emitted by the external tokenizer (both production + test mirror) before the line-start gate using `peek(-1)`/`peek(1)` lookahead that `@tokens` can't express; `a + b`/`c++` are not hard breaks. **T019/T020/T021 are now COMPLETE — the full inline-construct contract is implemented.** TDD: `asciidoc-grammar-us7-inline.test.ts` (56 cases incl. negatives/tricky-prose/zero-`⚠`), 11 highlight-consistency cases, e2e `editor-highlighting.spec.ts` (2 tests, 4/4 live). 623 codemirror + 1843 web unit + full e2e 119/119 green; gate clean.
- [X] T020 [US7] Mapped block nodes to `@lezer/highlight` tags; **inline/break nodes now mapped too** (`ThematicBreak`→contentSeparator, `PageBreak`→special(contentSeparator), `Passthrough`→special(string), `InlineAnchor`→labelName, `BiblioAnchor`→special(labelName), `Replacement`→character, `Entity`→special(character), `Callout`→special(number)); added `t.character` to `asciidoc-theme.ts` so all five themes colour replacements/entities. Highlight-consistency test extended (8 new cases assert each new construct resolves to a non-empty class). **DEFERRED with the remaining T019 constructs:** their theme entries.
- [X] T021 [P] [US7] Playwright spec `apps/web/e2e/editor-highlighting.spec.ts` extended with a second test covering the new inline + break constructs; **executed in the isolated stack — 4/4 green.** Precise per-construct class assertions still await semantic `class:` entries (deferred with the remaining constructs).
...
- [X] T078 Quality gate (build/lint/tsc/`fresh-onion`/`pnpm audit`) green; `pnpm audit` clean except the pre-existing `esbuild` dev advisory (identical on main, never shipped). Per-package `jest --coverage` against the 90/90/90/90 thresholds: **domain 96.7/90.7/97.5/97.2 ✅, api 94.8/90.2/92.1/95.0 ✅, shared 100/100/100/100 ✅** (api needed `refactoring-error-mapping.test.ts` to cover the defensive 500 branches → refactoring.ts now 100%). **New 026 modules carry their own coverage:** `file-name.ts` 100% (both copies), `editor-symbol-refactor.tsx` 93.7/90.5/93.3/98.5, `use-project-symbol-index.ts` 88.7/84.7/75/91.5 (refresh + getFiles now tested), `RenameSymbolUseCase`/`FindReferencesUseCase` ~97/100. **web global is 89.9/84.7/86.1/91.6 — below 90, a PRE-EXISTING app-wide condition** (the quality gate never ran web coverage; `project-editor-layout.tsx` and many non-026 components/hooks carry long-standing gaps). **✅ web global NOW CLOSED to 90/90/90/90** (2026-06-14): added ~377 unit tests across the previously-uncovered modules — pure (`extraction.ts`, `sandbox-path.ts`), CodeMirror state/view wiring (`table-context`, `format-keymap`, `attribute-fold`, `heading-levels`, `fold-persist`, `source-highlight`, `paste`, `spellcheck`, `fold`, `link-handler`, `completions`, `diagnostics`), the `use-editor-mount` hook, the API clients (`api/file-tree.ts`, `api/projects.ts`), and the big React components (`project-editor-layout.tsx`, `asciidoc-editor.tsx`, the file-tree components). Live-CM wiring is exercised with real `EditorView`s in jsdom (via a `/* @jest-environment jsdom */` pragma on `.test.ts` files) or built-from-grammar parsers. **Web global: statements 97.46 / branches 94.17 / functions 95.62 / lines 98.29 — all ≥90** (2220 web unit tests). **Enforced**: `scripts/ci/unit.sh` now runs web tests with `--coverage` (was bare `jest`), so the gate fails if web drops below threshold. domain 96.7/90.7/97.5/97.2, api 94.8/90.2/92.1/95.0, shared 100×4 all ✅. T078 done.
...
- [~] T080 Quickstart per-increment validation (A→D) + requirements checklist. **Requirements quality checklist (`checklists/requirements.md`) is fully `[x]`.** **Automated proxy COMPLETE:** every quickstart A→D scenario maps to a passing Playwright spec in the e2e coverage matrix — full suite **119/119 green on a clean stack** (A: preview-toggle, line-wrap; B: highlighting, folding, header-levels; C: insert-source, source-highlight, conveniences, metrics, fold-all; D: intelligence, refactoring, preview-includes, security-boundary). **REMAINING: the manual human acceptance pass** — a person exercising the running app per A→D to confirm subjective UX/feel. This is human-only by definition and cannot be automated; left for the maintainer. All other gates green: build/lint/tsc/fresh-onion + unit (shared 37, domain 736, api 459, web 2220 with coverage 90/90/90/90 enforced).
