...
- **Insufficient role**: Renaming requires the project-scoped editor/owner role. If the author lacks it, the apply is denied wholesale (and audit-logged) rather than partially applied; the suggestion may still be shown, but apply reports the denial.
- **Concurrent-edit / write conflict**: If a file's usages cannot be rewritten because of a concurrent edit or write failure, the apply reports which files could not be updated rather than failing silently.
...
- **FR-019**: On apply, the system MUST report the outcome (how many references in how many files were updated) and surface any files that could not be updated (e.g. due to a concurrent-edit / write conflict). Authorization is **project-scoped**: the author either holds the editor/owner role required to rename — in which case the whole operation proceeds — or the operation is denied wholesale and audit-logged (see FR-018a). There is no per-file permission subset, so a partial failure means a write conflict, not a per-file permission denial.
- **FR-020**: The apply operation MUST be reversible by the author as a **single action** that restores every rewritten usage — across all affected files and collaborative (Hocuspocus) sessions — back to the old name. This is realized as one inverse-rename operation over the same file set (not a distributed transaction across independent collaborative documents); the guarantee is that a single undo action fully restores the prior names.
