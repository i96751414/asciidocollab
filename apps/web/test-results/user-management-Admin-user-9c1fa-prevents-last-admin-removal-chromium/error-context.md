...
- Name: user-management.spec.ts >> Admin user management (US3) >> last-admin protection prevents last admin removal
- Location: e2e/user-management.spec.ts:83:7
...
- generic [active] [ref=e1]:
  ...
  - generic [ref=e4]:
    - generic [ref=e5]:
      - link "AsciiDoCollab" [ref=e7] [cursor=pointer]:
        - /url: /dashboard
      - navigation [ref=e8]:
        - link "Projects" [ref=e9] [cursor=pointer]:
          - /url: /dashboard
        - link "Archived" [ref=e10] [cursor=pointer]:
          - /url: /dashboard/archived
    - generic [ref=e11]:
      - generic [ref=e12]:
        - heading "Dashboard" [level=1] [ref=e13]
        - generic [ref=e14]:
          - link "Create Project" [ref=e15] [cursor=pointer]:
      ...
      - main [ref=e18]:
        - generic [ref=e19]:
          - generic [ref=e20]:
            - heading "Your Projects" [level=2] [ref=e21]
            - generic [ref=e22]:
              - link "Archived projects" [ref=e23] [cursor=pointer]:
                - /url: /dashboard/archived
              - link "New Project" [ref=e24] [cursor=pointer]:
                - /url: /dashboard/projects/new
          - generic [ref=e25]:
            - img [ref=e27]
            - heading "No projects yet" [level=3] [ref=e29]
            - paragraph [ref=e30]: Create your first project to get started with collaborative documentation.
            - link "Create Project" [ref=e31] [cursor=pointer]:
              - /url: /dashboard/projects/new
```
...
> 90  |     const admins = body.users.filter((u) => u.isAdmin);
  ...
  124 | 
  125 |       // Check the removal preview first (should list the sole-owned project)
  126 |       const previewResp = await page.request.get(`${API_URL}/admin/users/${userId}/removal-preview`);
  127 |       const preview = await previewResp.json() as { projectsToTransfer: Array<{ id: string }> };
  128 |       expect(preview.projectsToTransfer.some((p) => p.id === projectId)).toBe(true);
  129 | 
  130 |       // Confirm removal via UI
  131 |       await page.goto('/dashboard/admin/users');
  132 |       const userRow = page.locator('tr').filter({ hasText: email });
  133 |       await userRow.getByRole('button', { name: /remove/i }).click();
  134 | 
  135 |       // Confirm the removal dialog
  136 |       await page.getByRole('button', { name: /confirm/i }).click();
  137 | 
  138 |       // The user should no longer appear in the list
  139 |       await expect(page.getByText(email)).not.toBeVisible({ timeout: 5000 });
  140 |     } finally {
  141 |       await loginAdminViaApi(page);
  142 |       await adminDeleteUserByEmail(page, email);
  143 |     }
  144 |   });
  145 | });
  146 | 
