...
/**
 * Marks a file-tree node that is currently open by OTHER users. Shows a small cluster of their
 * DiceBear avatars (the same avatar they carry on review comments and in the in-editor presence bar)
 * and reveals who on hover/focus via the accessible label + native title. Renders nothing when no
 * other user has the file open, so it never appears for the viewer's own file. Uses design tokens, so
  ...
  return (
    ...
    >
      {shown.map((participant) => (
        <ParticipantAvatar key={participant.clientId} participant={participant} size={16} />
      ))}
