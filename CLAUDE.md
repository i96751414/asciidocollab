...
**Status:** Phase 1 complete — monorepo scaffolded and domain layer built. See `specs/001-domain-layer-scaffold/plan.md` for the implementation plan and `docs/superpowers/specs/2026-05-26-asciidocollab-architecture-design.md` for the full architecture spec.
...
- **UUID IDs**: Extend the `Uuid` abstract base class. `Uuid.equals()` uses constructor comparison to prevent cross-type equality (e.g. `UserId` !== `AuditLogId` even with same UUID string).
