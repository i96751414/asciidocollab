...
describe('Asset entity', () => {
  ...
  test('rejects sizeBytes < 0', () => {
    expect(() => new Asset(assetId, projectId, 'file.png', '/file.png', MimeType.create('image/png'), -1n, null, uploadedAt, null)).toThrow();
  });

  test('accepts sizeBytes = 0 (zero-byte file)', () => {
    const asset = new Asset(assetId, projectId, 'file.png', '/file.png', MimeType.create('image/png'), 0n, null, uploadedAt, null);
    expect(asset.sizeBytes).toBe(0n);
  });

  test('accepts sizeBytes > 0', () => {
    const asset = new Asset(assetId, projectId, 'file.png', '/file.png', MimeType.create('image/png'), 1n, null, uploadedAt, null);
  ...
  test('version chain via parentId', () => {
    ...
    const v2 = new Asset(v2Id, projectId, 'logo.png', '/uploads/logo.png', MimeType.create('image/png'), 2048n, v1, uploadedAt, new Date());
  ...
  test('uploadedAt is set on creation', () => {
    const now = new Date();
    const asset = new Asset(assetId, projectId, 'file.png', '/file.png', MimeType.create('image/png'), 100n, null, now, null);
    expect(asset.uploadedAt).toBe(now);
  });

  test('updatedAt is null initially and set on update', () => {
    const asset = new Asset(assetId, projectId, 'file.png', '/file.png', MimeType.create('image/png'), 100n, null, uploadedAt, null);
    expect(asset.updatedAt).toBeNull();
  });
});
