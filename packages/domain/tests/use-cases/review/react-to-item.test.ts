import { ReactToItemUseCase } from '../../../src/use-cases/review/react-to-item';
...
describe('ReactToItemUseCase', () => {
  ...
  let useCase: ReactToItemUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = new ReactToItemUseCase(reviewRepo, reactionRepo, memberRepo, auditRepo);
  ...
  test('an editor adds a reaction and it is returned', async () => {
    const result = await useCase.execute(EDITOR, PROJECT, ROOT, { emoji: '👍' });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.value.reactions).toHaveLength(1);
      expect(result.value.reactions[0].emoji).toBe('👍');
      expect(result.value.reactions[0].userId.equals(EDITOR)).toBe(true);
    }
  });

  test('reacting twice with the same emoji toggles it off', async () => {
    await useCase.execute(EDITOR, PROJECT, ROOT, { emoji: '👍' });
    const result = await useCase.execute(EDITOR, PROJECT, ROOT, { emoji: '👍' });
    expect(result.success).toBe(true);
    if (result.success) expect(result.value.reactions).toHaveLength(0);
  });
  ...
  test('a missing item is rejected as not found', async () => {
    const result = await useCase.execute(EDITOR, PROJECT, MISSING, { emoji: '👍' });
    expect(result.success).toBe(false);
  ...
  test('a viewer is denied and the denial is audited', async () => {
    expect.assertions(3);
    const result = await useCase.execute(VIEWER, PROJECT, ROOT, { emoji: '👍' });
    expect(result.success).toBe(false);
