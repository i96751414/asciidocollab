...
/**
 * Environment-agnostic AsciiDoc include-assembly primitive.
 *
 * Pre-expands `include::target[]` directives into a single document before it reaches a renderer, so a
 * consumer can render the configured main document with its includes inlined. This is the SINGLE source
 * of truth for include semantics (tags/lines/leveloffset filters, conditional include-gating, cycle and
 * fan-out guards) shared by every rendering path, so those paths can never drift.
 *
 * It performs NO I/O and hard-codes NO path policy: file reads and the sandbox path-resolution boundary
 * are supplied by the caller through {@link AssembleIncludesDependencies}. EVERY target is routed
 * through the injected {@link SandboxPathResolver}: parent-traversal (`..`), absolute, remote
 * (`http(s)://`), percent-encoded, and otherwise out-of-project targets are rejected and never read —
 * they are replaced with a renderer "Unresolved directive" marker instead. Cycles and excessive depth
 * are guarded. This is the only place assembly crosses file boundaries, so it must never read a path the
 * boundary did not bless.
 *
 * Conditional preprocessor directives (`ifdef`/`ifndef`/`ifeval`) are evaluated here ONLY to GATE
 * INCLUDES: an `include::` wrapped by a conditional region that is inactive for the current
 * document-order attribute state is NOT expanded (its target is never read). The directive lines
 * themselves — and any content-level (non-include) conditional content — are left verbatim in the
 * assembled source so the renderer evaluates content-level conditionals natively with the seeded
 * attributes. Evaluation is `eval`-free (see {@link evaluateConditional}). Nesting, undefined
 * attributes, and unbalanced `endif`/unclosed `if` are tolerated without aborting the assembly.
 */

/** Reads a project-relative path's content, or null if unavailable. */
 ...
 * project-relative path, or rejects it. The concrete policy is supplied by the caller so app and package
 * consumers can enforce their own boundary without cross-importing.
 */
export type SandboxPathResolver = (fromPath: string, target: string) => SandboxResolution;

/**
 * Builds the passthrough placeholder emitted in place of a hidden include body (hide mode). The concrete
 * markup is renderer-specific, so it is injected rather than hard-coded here.
 */
export type IncludePlaceholderBuilder = (target: string) => string;

/** The I/O and path-policy seams the assembly primitive depends on. */
export interface AssembleIncludesDependencies {
  /** Returns a project-relative path's content, or null if unavailable. */
  readFile: ProjectFileReader;
  /** Resolves + sandbox-confines an include target relative to its referencing file. */
  resolveSandboxedPath: SandboxPathResolver;
  /** Builds the hidden-include placeholder block (only invoked in hide mode). */
  buildPlaceholder: IncludePlaceholderBuilder;
}

/** A directive that could not be safely assembled, with the reason it was rejected. */
export interface AssembledIncludeRejection {
  /** The project-relative path of the file containing the directive. */
  from: string;
  /** The raw include target. */
  target: string;
  /** Why it was not assembled: a sandbox rejection reason, or `not-found` / `cycle` / `depth` / `limit`. */
  reason: string;
}

/** One entry in the assembled-line → source-file provenance map. */
export interface AssembledSourceMapEntry {
...
export interface AssembleIncludesResult {
  ...
  unresolved: AssembledIncludeRejection[];
  /** Provenance map (assembled line → source file + line). Only present when `options.withSourceMap` was set. */
  sourceMap?: AssembledSourceMap;
}

/** Bounds and attribute-seeding options for {@link assembleIncludes}. */
export interface AssembleIncludesOptions {
  /** Maximum include nesting depth. */
  maxDepth?: number;
  /** Global ceiling on the total number of include expansions across the whole assembly. */
  maxExpansions?: number;
  /** Attribute state already in effect where the root's content begins (intrinsics + API attributes). */
  seedAttributes?: ReadonlyMap<string, string>;
  /** When false, suppress included bodies (hide mode) while preserving attribute state. */
  showIncludes?: boolean;
  /** When true, emit a provenance map (assembled line → source). */
...
// suffix-unset (`:name!:`). These are emitted verbatim in hide mode so the renderer sees them
// in document order and resolves attribute state correctly.
const ATTR_ENTRY_RE =
 ...
 * semantics that distinguish `*` from `**`:
 ...
 * @param dependencies - Injected I/O (`readFile`), sandbox path policy (`resolveSandboxedPath`), and
 *   hidden-include placeholder builder (`buildPlaceholder`). No path policy or I/O is hard-coded here.
 ...
 *   lines in the source — the renderer's intrinsics (`backend-html5`, `filetype-html`, `doctype-article`,
 ...
 *   passthrough placeholder block, while attribute-entry lines are still emitted so the renderer
 *   resolves attribute state correctly.
 * @returns The assembled content and the list of unresolved/rejected directives.
 */
export function assembleIncludes(
  rootPath: string,
  dependencies: AssembleIncludesDependencies,
  options: AssembleIncludesOptions = {},
): AssembleIncludesResult {
  const { readFile, resolveSandboxedPath, buildPlaceholder } = dependencies;
  ...
  let expansions = 0;
  const unresolved: AssembledIncludeRejection[] = [];
  ...
  // When false, hide included bodies and emit attribute-entry lines only.
  ...
  const sourceMapAccumulator: AssembledSourceMapEntry[] = [];
  ...
  // is preserved for downstream content while the body is suppressed.
  const expand = (
    path: string,
    ...
    emit: boolean,
    sources: AssembledSourceMapEntry[] | null,
  ): string => {
    ...
    // `endif`. The directive lines that open/close regions are still emitted verbatim for the renderer.
    ...
    const expandIncludeLine = (rawTarget: string, attributeList: string, directiveLine: number): string[] => {
      ...
      if (!resolved.ok) {
        ...
        if (!emit) return [];
        if (hideMode) {
          const block = buildPlaceholder(rawTarget);
      ...
      if (stack.includes(resolved.path)) {
        ...
        if (!emit) return [];
        if (hideMode) {
          const block = buildPlaceholder(resolved.path);
          if (sources !== null) for (let index = 0; index < block.split('\n').length; index++) sources.push({ path, sourceLine: directiveLine });
          return [block];
        }
        if (sources !== null) sources.push({ path, sourceLine: directiveLine });
      ...
      // above expands nothing, so it must not deplete the budget and gate off later valid includes.
      ...
      if (!emit || hideMode) {
        ...
        // the parent attributes all resulting lines to the include directive.
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
      ...
      // the renderer sees the conditional. In emit:false (hidden-subtree attribute scan) the directive
      ...
      // emit:true mode so the renderer still sees the conditional. In emit:false mode the line is
      ...
      if (!match) {
        ...
        // emitted verbatim so the renderer performs its own native join and `data-source-line` mapping
        ...
        // attributes defined ABOVE it (matching extraction / Asciidoctor): set/unset and inline
        ...
        if (emit) {
          ...
          // the enclosing offset to return to. The line itself is still emitted for the renderer.
          offset = applyLevelOffsetEntry(line, offset, baseOffset);
          ...
          // values in the preview rather than raw `{attr}` placeholders.
          ...
          offset = applyLevelOffsetEntry(line, offset, baseOffset);
          if (isAttributeEntry) {
            // Attribute-entry lines are always emitted so the renderer sees them in document order.
            out.push(line);
      ...
      // the line is left verbatim so the renderer sees the directive (it cannot read the path anyway).
      // In emit:false (hidden-subtree attribute scan) the line is suppressed: a raw include:: from a
      // suppressed child must not surface in the assembled output where the renderer would try to
      // resolve it in the parent context. Target is never resolved or read in either case.
